let rowCounter = 1;
      ...
      let historyStack = [];
      let historyIndex = -1;
      let rateColumnHidden = false; // New variable to track rate column visibility
      ...
      function addRow() {
        ...
        const row1 = createTableRow(id, itemName, width, height, depth, measurement, notes, area, rate, quantity, amount);
        const row2 = createTableRow(id, itemName, width, height, depth, measurement, notes, area, rate, quantity, amount);
        ...
        // Clear input fields
        ["width", "height", "depth", "rate", "quantity", "itemName", "itemNotes"].forEach(id => {
      ...
      function updateRow() {
        ...
        // Construct the new particulars HTML
        let particularsHtml = `<div class="itemNameClass">${itemName}</div>`;
        let dimensionsParts = [];
        if (width) {
            dimensionsParts.push(`W ${width}${measurement}`);
        }
        if (height) {
            dimensionsParts.push(`H ${height}${measurement}`);
        }
        if (depth) { // Only add depth if it has a value
            dimensionsParts.push(`D ${depth}${measurement}`);
        }
        ...
        rows1.forEach(row => {
          ...
          cells[1].innerHTML = particularsHtml; // Update item name, sizes, and notes
      ...
      function createTableRow(id, itemName, width, height, depth, measurement, notes, area, rate, quantity, amount) {
      ...
      function editRow(id) {
        ...
        // This regex needs to be more flexible to handle missing dimensions
        ...
        if (dimensionsMatch) {
            ...
            document.getElementById("selectMeasurement").value = dimensionsMatch[2] || 'ft'; // Default to 'ft' if not found
        } else {
             // Clear dimensions inputs if no dimensions string is found
             document.getElementById("width").value = "";
             document.getElementById("height").value = "";
             document.getElementById("depth").value = "";
             document.getElementById("selectMeasurement").value = "ft"; // Reset to default
        }
        ...
        document.getElementById("itemNotes").value = notesText;
      ...
      function saveToLocalStorage() {
        ...
        document.querySelectorAll('#createList tbody tr[data-id]').forEach(row => {
          ...
          data.items.push({
            ...
            particulars: cells[1].innerHTML, // Store item name, sizes, and notes HTML
      ...
      function loadFromLocalStorage() {
        ...
        data.items.forEach(row => {
            ...
            row1.addEventListener('click', () => editRow(row.id));
            row1.innerHTML = `
              <td class="sr-no"></td>
              ...
              <td><button onclick="removeRow('${row.id}')" class="remove-btn">Remove</button></td>
            `;

            const row2 = document.createElement("tr");
            row2.setAttribute("data-id", row.id);
            row2.addEventListener('click', () => editRow(row.id));
            row2.innerHTML = `
              <td class="sr-no"></td>
              <td>${row.particulars}</td>
              <td>${row.area}</td>
      ...
      function downloadPDF() {
        ...
        // Also hide the Rate column if it's currently visible
        if (!rateColumnHidden) {
            hideTableColumn(document.getElementById("createList"), 3, "none");
            hideTableColumn(document.getElementById("copyList"), 3, "none");
        }
        ...
        html2pdf().set(opt).from(billContainer).save().then(() => {
            ...
            setTimeout(() => {
                ...
                // Restore the Rate column if it was visible before PDF generation
                if (!rateColumnHidden) {
                    hideTableColumn(document.getElementById("createList"), 3, "table-cell");
                    hideTableColumn(document.getElementById("copyList"), 3, "table-cell");
                }
      ...
      function saveStateToHistory() {
        ...
        const state = {
          items: Array.from(document.querySelectorAll('#createList tbody tr[data-id]')).map(row => {
            ...
            return {
              id: row.dataset.id,
              particulars: cells[1].innerHTML, // Save the full HTML of the particulars cell
      ...
      // Function to automatically save the current state
      function autoSave() {
          saveToLocalStorage();
          saveStateToHistory();
      }

      // Function to toggle the visibility of the "Rate" column
      function toggleRateColumn() {
        const rateColumnIndex = 3; // Index of the 'Rate' column (0-indexed)
        ...
        if (rateColumnHidden) {
          // Show the Rate column
          hideTableColumn(document.getElementById("createList"), rateColumnIndex, "table-cell");
          hideTableColumn(document.getElementById("copyList"), rateColumnIndex, "table-cell");
          // Update the icon and text
          icon.textContent = "visibility_off";
          toggleButton.innerHTML = `<span class="material-icons">visibility_off</span>RATE`;
        } else {
          // Hide the Rate column
          hideTableColumn(document.getElementById("createList"), rateColumnIndex, "none");
          hideTableColumn(document.getElementById("copyList"), rateColumnIndex, "none");
          // Update the icon and text
          icon.textContent = "visibility";
          toggleButton.innerHTML = `<span class="material-icons">visibility</span>RATE`;
        }
        rateColumnHidden = !rateColumnHidden; // Toggle the state
      }
